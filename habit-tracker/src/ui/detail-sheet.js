  ...
  function renderDetailSheet(
    ...
    const recentLogsHtml = recentLogs
      .map(({ date, log, prog }) => {
        ...
        return `
          <div class="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700/40 mb-2 flex flex-col gap-1">
            <div class="flex items-center justify-between">
              <span class="text-xs font-semibold text-slate-700 dark:text-slate-300">${i18n.formatDate(date, lang, "full")}</span>
    ...
    const html = `
      <div class="sheet-card bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-t-3xl sm:rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 shadow-2xl text-slate-900 dark:text-white" role="dialog" aria-modal="true" aria-labelledby="detail-sheet-title">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <span class="text-xs text-slate-500 dark:text-slate-400">${i18n.t(`routine_${habit.routine || "anytime"}`, {}, lang)} • ${i18n.t(`type_${habit.type || "binary"}`, {}, lang)}</span>
            </div>
          </div>
          <button type="button" data-action="close-detail-sheet" class="text-slate-400 hover:text-slate-700 dark:hover:text-white text-2xl leading-none">&times;</button>
        </div>
        ...
        <div class="mb-5">
          <h4 class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">${i18n.t("mini_heatmap_365", {}, lang)}</h4>
          ${miniHeatmapHtml}
        </div>
