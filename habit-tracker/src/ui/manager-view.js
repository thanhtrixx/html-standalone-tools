...
(function (global) {
  ...
  function renderHabitEditModal(habit = null, lang = "vi") {
    ...
    return `
      <div id="habit-edit-modal-backdrop" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 transition-opacity">
        <div class="modal-card bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 shadow-2xl text-white">
          <div class="flex items-center justify-between pb-4 border-b border-slate-800">
            <h3 class="text-xl font-bold">${modalTitle}</h3>
            <button type="button" data-action="close-modal" class="text-slate-400 hover:text-white text-2xl leading-none">&times;</button>
          </div>
          ...
          <form id="habit-edit-form" data-habit-id="${habitId}" class="space-y-4 pt-4">
            ...
            <div class="flex items-center gap-3">
              <div class="w-16">
                <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("icon_emoji_label", {}, lang)}</label>
                <input type="text" id="modal-habit-icon" name="icon" value="${icon}" class="w-full text-center text-xl bg-slate-800 border border-slate-700 rounded-xl py-2 px-1 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              </div>
              <div class="flex-1">
                <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("habit_name_label", {}, lang)} *</label>
                <input type="text" id="modal-habit-name" name="name" value="${name}" placeholder="${i18n.t("habit_name_placeholder", {}, lang)}" required class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              </div>
            </div>

            <!-- Measurement Type -->
            <div>
              <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("habit_type_label", {}, lang)}</label>
              <div class="grid grid-cols-3 gap-2">
                <label class="flex flex-col items-center p-2 rounded-xl bg-slate-800/80 border border-slate-700 cursor-pointer text-xs hover:bg-slate-700/50">
            ...
            <div id="modal-target-fields" class="grid grid-cols-3 gap-3 ${type === "binary" ? "hidden" : ""}">
              <div>
                <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("target_value_label", {}, lang)}</label>
                <input type="number" id="modal-target-value" name="targetValue" value="${targetValue}" min="1" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              </div>
              <div>
                <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("target_unit_label", {}, lang)}</label>
                <input type="text" id="modal-target-unit" name="unit" value="${unit}" placeholder="ml, pages, mins" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              </div>
              <div>
                <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("step_delta_label", {}, lang)}</label>
                <input type="number" id="modal-step" name="step" value="${step}" min="1" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              </div>
            </div>

            <!-- Routine Assignment -->
            <div>
              <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("routine_label", {}, lang)}</label>
              <select name="routine" id="modal-habit-routine" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none text-white">
            ...
            <!-- Frequency Schedule -->
            <div>
              <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("schedule_label", {}, lang)}</label>
              <select name="scheduleType" id="modal-schedule-type" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none text-white mb-2">
              ...
              <div id="modal-interval-container" class="flex items-center gap-2 pt-1 ${scheduleType === "interval" ? "" : "hidden"}">
                <span class="text-xs text-slate-400">Every</span>
                <input type="number" name="intervalDays" value="${intervalDays}" min="1" max="30" class="w-20 bg-slate-800 border border-slate-700 rounded-xl py-1.5 px-3 text-sm text-center">
            ...
            <!-- Color Theme Pills -->
            <div>
              <label class="block text-xs font-semibold text-slate-400 mb-2">${i18n.t("color_theme_label", {}, lang)}</label>
              <div class="flex items-center gap-3">
                ${colorPillsHtml}
              </div>
            </div>
            ...
            <!-- Daily Reminder Time -->
            <div>
              <label class="block text-xs font-semibold text-slate-400 mb-1">${i18n.t("reminder_time_label", {}, lang)}</label>
              <input type="time" name="reminderTime" value="${reminderTime}" class="bg-slate-800 border border-slate-700 rounded-xl py-2 px-3 text-sm text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none">
            </div>

            <!-- Action Buttons -->
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
              <button type="button" data-action="close-modal" class="px-4 py-2 text-sm text-slate-400 hover:text-white rounded-xl">
                ${i18n.t("cancel", {}, lang)}
              </button>
  ...
  function renderManagerView(store, containerElement, lang = "vi") {
    ...
    const routineSectionsHtml = routineKeys
      .map((rKey) => {
            ...
            return `
              <div class="manager-habit-card bg-slate-900/90 border border-slate-800/90 rounded-2xl p-4 mb-3 flex items-center justify-between gap-3 shadow-md" data-habit-id="${h.id}">
                <div class="flex items-center gap-3 flex-1">
                  ...
                  <div>
                    <h4 class="font-semibold text-white text-base">${h.name}</h4>
                    <span class="text-xs text-slate-400">${i18n.t(`type_${h.type || "binary"}`, {}, lang)}</span>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  <!-- Archive -->
                  <button type="button" data-action="archive-habit" data-habit-id="${h.id}" class="p-1.5 rounded-lg bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 text-sm" title="${i18n.t("archive_habit", {}, lang)}">📦</button>
                  <!-- Delete -->
                  <button type="button" data-action="delete-habit" data-habit-id="${h.id}" class="p-1.5 rounded-lg bg-slate-800 text-rose-400 hover:bg-rose-950 text-sm" title="${i18n.t("delete", {}, lang)}">🗑️</button>
                </div>
              </div>
            `;
          })
        ...
        return `
          <div class="mb-6">
            <h3 class="text-sm font-bold uppercase tracking-wider text-slate-400 mb-2 px-1">${i18n.t(`routine_${rKey}`, {}, lang)}</h3>
            <div>${itemsHtml}</div>
          </div>
        `;
      })
        ...
        <div class="mt-8 pt-6 border-t border-slate-800">
          ...
          <div class="space-y-2">
            ${archivedHabits
              .map(
                (h) => `
                <div class="bg-slate-900/60 border border-slate-800/50 rounded-xl p-3 flex items-center justify-between text-slate-400">
                  <div class="flex items-center gap-2">
                    <span>${h.icon || "🎯"}</span>
                    <span class="line-through text-sm">${h.name}</span>
                  </div>
                  <button type="button" data-action="restore-habit" data-habit-id="${h.id}" class="text-xs bg-slate-800 text-slate-300 hover:text-white px-3 py-1 rounded-lg">
    ...
    const html = `
      <div class="manager-view max-w-lg mx-auto pb-24">
        <div class="flex items-center justify-between mb-6 px-1">
          <div>
            <h2 class="text-2xl font-black text-white">${i18n.t("manager_tab", {}, lang)}</h2>
