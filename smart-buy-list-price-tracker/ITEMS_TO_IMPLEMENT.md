# 📋 Smart Buy-List & Unit Price Tracker — Living Requirements & Specifications (v3.12.0)
...
|                                               | **CAP-06** | Horizontal aisle department quick filter chips and available-only Shopping List store filter chips and top dropdown (`All Stores`, active stores with $\ge 1$ items, clean emoji-free labels, `Manage Stores`).                                                                                                              | ADR-0019, ADR-0020, CONTEXT.md                   |
| **3. Trip Lifecycle & Offline Persistence**   | **CAP-07** | 3-Phase shopping trip lifecycle: Planning mode (rich expanded cards, Smart Omnibox), In-Store Buy Mode (minimalist cards, running total, pacing bar), unified `#finishTripBar` requiring `checkedItems.length > 0` across both modes, guarded completion modal, and Trip Summary/Complete (ledger logging, rollover).        | ADR-0004, ADR-0006, ADR-0017, ADR-0018, ADR-0021 |
...
| **6. PWA Shell & Ergonomics**                 | **CAP-17** | Single-source PWA versioning (`3.12.0`) anchored in `manifest.webmanifest`, Network-First HTML navigation with M3 update notification toast, cache purge controls, and companion asset compaction (Terser minified `sw.js`).                                                                                                 | ADR-0007, ADR-0015, ADR-0018, ADR-0020, ADR-0021 |
...
|                                               | **CAP-19** | Smart Quick-Entry Omnibox (`#smartQuickInput`) with inline store selector pill (`#smartQuickStoreSelect`), filter context inheritance, real-time NLP parsing (items, quantities, units, prices, `@store`, categories), 1-tap add, undo toast, and multi-line batch paste.                                                    | ADR-0013, ADR-0020                               |
