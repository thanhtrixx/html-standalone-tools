# 📋 Smart Buy-List & Unit Price Tracker — Living Requirements & Specifications (v3.9.0)
...
- **Storage Provider Seam**: `IndexedDBStorageProvider` (offline engine), `GoogleDriveStorageProvider` (AppData REST v3), `GitHubGistStorageProvider` (Secret Gist PAT), and Deterministic 3-Way Cloud Merge (`Merge3`) with deletion tombstones.
...
|                                               | **CAP-06** | Horizontal aisle department quick filter chips (`All`, `Produce`, `Dairy`, `Meat`, `Bakery`, `Pantry`, `Frozen`, `Beverages`, `Household`, `Personal Care`, `Other`).                                                                                                                         | CONTEXT.md                             |
...
|                                               | **CAP-09** | Ledger-to-BuyList re-ordering via 1-tap quick add (`➕`) and multi-select batch restocking with sticky action bar, case-insensitive consolidation, and actionable toast.                                                                                                                      | ADR-0008, ADR-0017                     |
| **4. Multi-Cloud Sync & Conflict Resolution** | **CAP-10** | Pluggable `StorageManager` registry supporting `Disabled (Local Only)`, `Google Drive (AppData GIS OAuth)`, and `GitHub Gist (Secret Gist PAT)` cloud engines.                                                                                                                                | ADR-0011, ADR-0012                     |
...
|                                               | **CAP-13** | Detailed diagnostic error interpolation (`res.error                                                                                                                                                                                                                                           |                                        | "Unknown"`) across cloud synchronization failure toasts in English and Vietnamese. | ADR-0018           |
...
| **6. PWA Shell & Ergonomics**                 | **CAP-17** | Single-source PWA versioning (`3.9.0`) anchored in `manifest.webmanifest`, Network-First HTML navigation with M3 update notification toast, cache purge controls, and companion asset compaction (Terser minified `sw.js`).                                                                   | ADR-0007, ADR-0015, ADR-0018           |
...
|                                               | **CAP-20** | Vietnam-first baseline defaults (`language: 'vi'`, `currency: 'VND'`), flag emoji switcher (`🇻🇳`/`🇺🇸`), currency-aware quick price adjustment delta chips, and 100% bilingual dictionary parity.                                                                                              | ADR-0013, ADR-0014                     |
