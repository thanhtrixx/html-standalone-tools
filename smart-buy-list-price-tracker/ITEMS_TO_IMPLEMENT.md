# 📋 Smart Buy-List & Unit Price Tracker — Living Requirements & Specifications (v3.11.0)
...
- **Option Hub (Settings)**: Symmetrical 2x2 data interchange grid (`[Export File | Copy JSON] / [Import File | Paste JSON]`), 2-button cloud override actions, store management, preferences, and cloud sync.
- **Storage Provider Seam**: `IndexedDBStorageProvider` (offline engine), `GoogleDriveStorageProvider` (AppData REST v3), `GitHubGistStorageProvider` (Secret Gist PAT with 403 rate limit diagnostics and Classic scope enforcement), and Deterministic 3-Way Cloud Merge (`Merge3`) with deletion tombstones.
...
|                                               | **CAP-13** | Detailed diagnostic error interpolation and GitHub API rate limit calculation (`Resets at HH:MM`) across cloud synchronization failure toasts.                                                                                                                                                  | ADR-0018, ADR-0019, ADR-0020           |
...
| **6. PWA Shell & Ergonomics**                 | **CAP-17** | Single-source PWA versioning (`3.11.0`) anchored in `manifest.webmanifest`, Network-First HTML navigation with M3 update notification toast, cache purge controls, and companion asset compaction (Terser minified `sw.js`).                                                                    | ADR-0007, ADR-0015, ADR-0018, ADR-0020 |
