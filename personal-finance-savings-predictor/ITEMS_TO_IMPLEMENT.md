...
> **Source Documents:** `CONTEXT.md`, `docs/adr/`  
...
- **Flexible Pool**: Primary liquid cash balance receiving monthly salary deposits, paying out scheduled withdrawals, and earning demand interest. Permits temporary deficit balance with explicit simulation warning events.
...
- **Auto 6M Deposit**: Automatic 6-month Fixed Term Deposit triggered whenever the Flexible Pool balance is $\ge$ 200,000,000 VND. On maturity, principal and interest return to the Flexible Pool and immediately re-evaluate the threshold on that date.
...
- **Salary Escalation**: Annual compound percentage increase applied to base monthly salary on each 12-month anniversary from the simulation start date.
...
- **Scenario Comparison**: Side-by-side full simulation runs comparing Scenario A (baseline) against Scenario B (custom/projected parameters).
...
| ------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------- |
| **R1**  | CSV Persistence               | Synchronously save parsed CSV portfolio data to `localStorage` (`workingCSVData`) upon edit/run.                                              | P0       | ADR-0001      |
| **R2**  | Parameter Persistence         | Save/load all simulation parameters (`params`) in `localStorage` with versioned schema migration (`SCHEMA_VERSION = 1`).                      | P0       | ADR-0001      |
| **R3**  | Reset Defaults                | Restore default portfolio, parameters, and UI settings without blocking prompts.                                                              | P1       | —             |
| **R4**  | Salary Escalation             | Deposit monthly salary on the 1st of each month; step up base salary by growth rate every 12-month anniversary from start date.               | P0       | ADR-0003      |
...
| **R6**  | Scheduled Withdrawals         | Deduct scheduled withdrawals from Flexible Pool. If withdrawal exceeds liquid balance, allow deficit and emit `DEFICIT` warning log.          | P0       | ADR-0002      |
| **R7**  | Goal Tracking & Milestone     | Display progress bar + ring chart when Savings Goal $> 0$, and project the exact Milestone Date.                                              | P1       | —             |
| **R8**  | Shareable Link                | Serialize simulation state and portfolio CSV into URL hash via URL-safe Base64; auto-load state when visiting hash URL.                       | P1       | ADR-0001      |
...
| **R17** | Scenario Comparison Engine    | Run an actual dual-pass simulation for Scenario B with independent parameters (not hardcoded multipliers).                                    | P1       | —             |
...
| **R20** | Auto 6M Allocation Rule       | Lock 200M VND chunks from Flexible Pool into 6-month Fixed Term Deposits; deduct locked funds from pool immediately to avoid double-counting. | P0       | ADR-0002      |
| **R21** | Bilingual Localization (i18n) | Support English (`en`) and Vietnamese (`vi`) with complete text translations, `₫` vs `VND` currency formatting, and localized date formats.   | P1       | ADR-0004      |

---

## ⚙️ Calculation & Engine Logic Specifications

### 1. Daily Simulation Timeline
...
2. **Auto 6M Evaluation (Step 1)**: If Flexible Pool balance $\ge 200,000,000 \text{ VND}$:
   ...
   - Record `AUTO_6M` event in simulation logs.
3. **Salary Inflow**: If day $t$ is the 1st of the month:
...
4. **Withdrawal Outflows**: If scheduled withdrawal exists for day $t$:
   - Deduct withdrawal amount from Flexible Pool.
   - If Flexible Pool $< 0$, record `DEFICIT_WARNING` in simulation logs.
   - Record `WITHDRAWAL` event in simulation logs.
5. **Demand Interest Accrual**:
   ...
   - If day $t$ is the last day of the month, credit accrued demand interest to Flexible Pool and record `INTEREST_POOL` log.
6. **Milestone Check**:
