...
- **Auto Term Deposit**: Automatic fixed-term savings deposit triggered whenever the Flexible Pool balance meets or exceeds the configurable Auto Term Threshold ($\ge$ 200,000,000 VND by default). The entire available liquid pool balance is locked into a single term deposit of configurable duration ($N$ months) and annual rate, setting the liquid Flexible Pool balance to 0. On maturity, principal and interest return to the Flexible Pool and immediately re-evaluate the threshold on that date ([ADR-0005](./docs/adr/0005-unified-threshold-auto-term-allocation.md)).
...
| **R2**  | Parameter Persistence         | Save/load all simulation parameters (`params`) in `localStorage` with versioned schema migration (`SCHEMA_VERSION = 2`).                    | P0       | Global ADR-0001 |
...
| **R20** | Auto Term Allocation Rule     | When Flexible Pool $\ge$ Auto Term Threshold ($>0$), sweep entire pool balance into a single Fixed Term Deposit of duration $N$ months.     | P0       | ADR-0005        |
...
5. **Unified Auto Term Threshold Rule ([ADR-0005](./docs/adr/0005-unified-threshold-auto-term-allocation.md))**:
   - If $\text{Auto Term Threshold} > 0$ and $\text{Flexible Pool balance} \ge \text{Auto Term Threshold}$:
     - Set $\text{Locked Amount} = \text{Flexible Pool balance}$.
     - Set $\text{Flexible Pool balance} = 0$.
     - Create **exactly one** new Fixed Term Deposit starting on day $t$ with end date $t + \text{autoTermMonths}$ months and rate $\text{autoTermAnnualRate}$.
