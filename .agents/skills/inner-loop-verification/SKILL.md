...
description: "Sub-second, tool-scoped inner loop verification skill. Enforces strict execution of tool-scoped test suites (npm run test:<tool>), grep-first file navigation, and scoped formatting checks while strictly prohibiting whole-repo bun run verify in inner loops."
---
...
1. **Always use tool-scoped runners** (`npm run test:<tool>`).
...
1. Grep-First Navigation  ──►  2. Surgical Edit  ──►  3. Scoped Verification
   grep -n "function"            replace_file_content     npm run test:<tool>
   (Pinpoint exact line)         (Targeted diff block)    (<0.1s, ~15 lines output)
```
...
- Execute *only* the unit/DOM suite for the tool under active development:

| Tool Directory | Inner-Loop Unit/DOM Command | Execution Time | Output Size |
| :--- | :--- | :--- | :--- |
| `buy-vs-rent-home-comparison/` | `npm run test:buy-rent` | ~0.08s | ~15 lines |
| `english-shadowing/` | `npm run test:shadowing` | ~0.06s | ~15 lines |
| `habit-tracker/` | `npm run test:habit` | ~0.08s | ~18 lines |
| `personal-finance-savings-predictor/` | `npm run test:sim` | ~0.05s | ~12 lines |
| `smart-buy-list-price-tracker/` | `npm run test:tracker` | ~0.09s | ~20 lines |
| `portal/` | `npm run test:portal` | ~0.04s | ~10 lines |
...
2. ❌ **STRICTLY PROHIBITED**: Running full-repo `prettier --check .` during inner loops. Use scoped format checks:
   ```bash
   npx prettier --check <tool-directory>/**
   ```
3. ❌ **STRICTLY PROHIBITED**: Speculatively viewing >200 lines of files without line-number discovery via `grep -n`.
4. ❌ **STRICTLY PROHIBITED**: Running raw un-aggregated `playwright test` in inner loops (use `npm run test:e2e:<tool>` or `scripts/e2e-summary.js`).
...
1. Run scoped E2E if tool is `Hardened Stable`: `npm run test:e2e:<tool>`.
2. Run format fix: `npm run format`.
